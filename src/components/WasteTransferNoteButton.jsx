...
export default function WasteDocumentButton({ facility, ewcCode, ewcDescription, isHazardous }) {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // WASTE TRANSFER NOTE — matches EA form LIT_7932 (WMC2A Version 3)
  // A4 portrait, margin 10mm each side, all sections sequential, no overlaps
  // ═══════════════════════════════════════════════════════════════════════════
  const generateWTN = (doc, W, margin) => {
    // ── Layout constants ──────────────────────────────────────────────────
    const cW   = W - margin * 2;          // 190 mm usable width
    const half = (cW - 1) / 2;            // ~94.5 mm each column
    const c2   = margin + half + 1;       // x-start of right column
    const P    = 1.5;                      // inner padding
    const LH   = 4.2;                     // label-to-value line height
    const SH   = 4.5;                     // section bar height

    // ── Helpers ───────────────────────────────────────────────────────────
    const sf  = (style='normal', sz=7)  => { doc.setFont('helvetica', style); doc.setFontSize(sz); };
    const sc  = (r,g,b)                 => doc.setTextColor(r,g,b);
    const rc  = ()                      => doc.setTextColor(20,20,20);
    const dc  = (r,g,b,lw=0.3)         => { doc.setDrawColor(r,g,b); doc.setLineWidth(lw); };
    const box = (x,y,w,h)              => { dc(120,120,120,0.3); doc.rect(x,y,w,h); };
    const bg  = (x,y,w,h,r,g,b)        => { doc.setFillColor(r,g,b); doc.rect(x,y,w,h,'F'); };
    const hln = (x1,y,x2)              => { dc(170,170,170,0.25); doc.line(x1,y,x2,y); };
    const vln = (x,y1,y2)              => { dc(170,170,170,0.25); doc.line(x,y1,x,y2); };

    // Small grey label above data
    const lbl = (txt,x,y,mw=80) => {
      sf('normal',5.5); sc(90,90,90);
      doc.text(txt, x, y, { maxWidth: mw });
      rc();
    };
    // Bold data value
    const val = (txt,x,y,mw=80) => {
      sf('bold',7); rc();
      if (txt) doc.text(String(txt), x, y, { maxWidth: mw });
      sf('normal',7);
    };
    // Tick-box — solid border box with crisp X drawn as two diagonal lines
    const tick = (x,y,on) => {
      dc(60,60,60,0.4); doc.rect(x,y,3.5,3.5);
      if (on) {
        doc.setDrawColor(20,20,20); doc.setLineWidth(0.7);
        doc.line(x+0.5, y+0.5, x+3.0, y+3.0);
        doc.line(x+3.0, y+0.5, x+0.5, y+3.0);
        dc(60,60,60,0.4);
      }
    };
    // Green section bar — returns new y below bar
    const secBar = (txt,x,y,w) => {
      bg(x,y,w,SH, 30,90,50);
      sf('bold',6.5); sc(255,255,255);
      doc.text(txt, x+P, y+3.3);
      rc(); return y + SH;
    };
    ...
    // HEADER
    // ═══════════════════════════════════════════════════════════════════════
    bg(0,0,W,13, 30,90,50);
    sf('bold',10.5); sc(255,255,255);
    doc.text('Duty of care: waste transfer note', margin, 7.5);
    sf('normal',5.5); sc(180,220,180);
    doc.text('Keep this page and copy it for future use  ·  WMC2A Version 3, August 2011  ·  Environmental Protection Act 1990, s.34', margin, 11);
    sf('bold',5.5); sc(255,240,150);
    doc.text('WasteLocate.co.uk  ·  Generated: '+today, W-margin, 11, { align:'right' });
    rc();
    ...
    // A1 — full-width description + EWC code side by side
    const a1H = 12;
    box(margin, y, half, a1H);
    lbl('A1 Description of the waste being transferred', margin+P, y+3.5);
    val(form.wasteDescription || ewcDescription || '', margin+P, y+8, half-P*2);

    box(c2, y, half, a1H);
    lbl('List of Waste Regulations code(s)', c2+P, y+3.5);
    sf('bold',11); sc(20,60,140);
    doc.text(ewcCode || '', c2+P, y+10);
    rc();
    y += a1H;

    // A2 + A3 — contained / quantity side by side
    const a2H = 11;
    box(margin, y, half, a2H);
    lbl('A2 How is the waste contained?', margin+P, y+3.5);
    let cx = margin+P;
    ['Loose','Sacks','Skip','Drum','Other'].forEach(c => {
      tick(cx, y+4.5, form.containmentType === c.toLowerCase() || (c==='Other' && form.containmentType==='other'));
      sf('normal',6); rc();
      doc.text(c, cx+4, y+7.2);
      cx += (c==='Other') ? 0 : 17;
    });
    if (form.containmentType==='other' && form.containmentOther) {
      sf('italic',5.5); sc(60,60,60);
      doc.text('('+String(form.containmentOther).slice(0,35)+')', margin+P, y+10, { maxWidth: half-P*2 });
      rc();
    }

    box(c2, y, half, a2H);
    lbl('A3 How much waste? For example, number of sacks, weight', c2+P, y+3.5, half-P*2);
    val(form.quantity || '', c2+P, y+8.5, half-P*2);
    y += a2H;

    // A4 — process giving rise to waste (full width)
    const a4H = 11;
    box(margin, y, cW, a4H);
    lbl('A4 Process giving rise to this waste', margin+P, y+3.5);
    val(form.wasteProcess || '', margin+P, y+8, cW-P*2);
    y += a4H;
    ...
    // B1 — name / company / address | tel / email / B2
    const b1H = 26;
    box(margin, y, half, b1H);
    lbl('B1 Full name', margin+P, y+3.5);
    val(form.transferorName, margin+P, y+7, half-P*2);
    hln(margin+P, y+8, margin+half-P);

    lbl('Company name and address', margin+P, y+11);
    val(form.transferorCompany, margin+P, y+14.5, half-P*2);
    val(form.transferorAddress,  margin+P, y+18.5, half-P*2);
    hln(margin+P, y+20, margin+half-P);

    lbl('Postcode', margin+P, y+22);
    val(form.transferorPostcode, margin+P, y+25.5, 28);
    vln(margin+half*0.5, y+21, y+b1H);
    lbl('SIC code (2007)', margin+half*0.5+P, y+22);
    val(form.transferorSic, margin+half*0.5+P, y+25.5, 20);

    box(c2, y, half, b1H);
    lbl('Telephone', c2+P, y+3.5);
    val(form.transferorTel, c2+P, y+7, half-P*2);
    hln(c2+P, y+8, c2+half-P);
    lbl('Email', c2+P, y+11);
    val(form.transferorEmail, c2+P, y+14.5, half-P*2);
    hln(c2+P, y+16, c2+half-P);
    lbl('B2 Name of your unitary authority or council', c2+P, y+19, half-P*2);
    y += b1H;

    // B3 — "Are you:" checkboxes — full width, 4 rows
    const b3H = 28;
    box(margin, y, cW, b3H);
    lbl('B3 Are you:', margin+P, y+3.5);

    // Row 1: producer / importer / local authority
    const b3r1 = y+6;
    [['The producer of the waste?','producer'],['The importer of the waste?','importer'],['The local authority?','local_authority']].forEach(([t,v],i) => {
      const bx = margin+P + i*63;
      tick(bx, b3r1, form.transferorType===v);
      sf('normal',6); rc(); doc.text(t, bx+4.2, b3r1+2.8);
    });

    // Row 2: permit holder
    const b3r2 = y+12;
    tick(margin+P, b3r2, form.transferorType==='permit_holder');
    sf('normal',6); rc(); doc.text('The holder of an environmental permit?', margin+P+4.2, b3r2+2.8);
    lbl('Permit number', margin+P+77, b3r2+0.5);
    val(form.transferorPermitNumber, margin+P+77, b3r2+4, 40);
    vln(margin+P+75, b3r2, b3r2+7);
    if (form.transferorPermitIssuedBy) {
      lbl('Issued by', margin+P+120, b3r2+0.5);
      val(form.transferorPermitIssuedBy, margin+P+120, b3r2+4, 45);
      vln(margin+P+118, b3r2, b3r2+7);
    }
    hln(margin+P, b3r2+7, margin+cW-P);

    // Row 3: registered exemption
    const b3r3 = y+20;
    tick(margin+P, b3r3, form.transferorType==='exemption');
    sf('normal',6); rc(); doc.text('Registered waste exemption?', margin+P+4.2, b3r3+2.8);
    lbl('Details, including registration number', margin+P+60, b3r3+0.5, 60);
    val(form.transferorType==='exemption' ? form.transferorRegNumber : '', margin+P+60, b3r3+4, 80);

    // Row 4: carrier/broker/dealer
    const b3r4 = y+24;
    tick(margin+P, b3r4, form.transferorType==='carrier');
    sf('normal',6); rc(); doc.text('A registered waste carrier, broker or dealer?', margin+P+4.2, b3r4+2.8);
    lbl('Registration number', margin+P+90, b3r4+0.5);
    val(form.transferorType==='carrier' ? form.transferorRegNumber : '', margin+P+90, b3r4+4, 55);
    y += b3H;

    // ═══════════════════════════════════════════════════════════════════════
    // SECTION C  – Carrier then Transferee (disposal site), all one section
    // ═══════════════════════════════════════════════════════════════════════
    y = secBar('Section C – Person collecting the waste – Transferee', margin, y, cW);

    // C-CARRIER — carrier details first (before disposal location)
    const carH = 20;
    box(margin, y, half, carH);
    lbl('Carrier name', margin+P, y+3.5);
    val(form.carrierName || '', margin+P, y+7.5, half-P*2);
    hln(margin+P, y+9, margin+half-P);
    lbl('Company name and address', margin+P, y+12);
    val(form.carrierCompany || '', margin+P, y+15.5, half*0.55);
    val(form.carrierAddress  || '', margin+P, y+19, half-P*2);

    box(c2, y, half, carH);
    lbl('Registration number / exemption reason', c2+P, y+3.5, half-P*2);
    val(form.carrierRegNumber || '', c2+P, y+7.5, half-P*2);
    hln(c2+P, y+9, c2+half-P);
    lbl('Vehicle registration', c2+P, y+12);
    val(form.vehicleReg || '', c2+P, y+15.5, half*0.5);
    vln(c2+half*0.55, y+11, y+carH);
    lbl('Postcode', c2+half*0.55+P, y+12);
    val(form.carrierPostcode || '', c2+half*0.55+P, y+15.5, half*0.4);
    y += carH;

    // C1 — disposal site: name / address / postcode (left) | C2 local authority / C3 permit (right)
    const c1H = 30;
    bg(margin, y, half, c1H, 245,252,245);
    box(margin, y, half, c1H);
    lbl('C1 Full name (disposal site)', margin+P, y+3.5);
    val(form.transfereeName || facility.operator_name || facility.name || '', margin+P, y+7.5, half-P*2);
    hln(margin+P, y+9, margin+half-P);

    lbl('Company name and address', margin+P, y+12);
    val(facility.address || '', margin+P, y+16, half-P*2);
    hln(margin+P, y+20, margin+half-P);

    lbl('Postcode', margin+P, y+23);
    val(facility.postcode || '', margin+P, y+27, 28);
    sf('italic',5); sc(40,110,50);
    doc.text('[Pre-filled – WasteLocate]', margin+P, y+c1H-1.5);
    rc();

    bg(c2, y, half, c1H, 245,252,245);
    box(c2, y, half, c1H);
    // C2
    lbl('C2 Are you:', c2+P, y+3.5);
    tick(c2+P, y+5.5, false);
    sf('normal',6); rc(); doc.text('The local authority?', c2+P+4.2, y+8.3);
    hln(c2+P, y+10, c2+half-P);

    // C3
    lbl('C3 Are you:', c2+P, y+12.5);
    tick(c2+P, y+14.5, true);
    sf('normal',6); rc(); doc.text('The holder of an environmental permit?', c2+P+4.2, y+17.3);
    lbl('Permit number', c2+P, y+20.5);
    val(form.transfereePermitNumber || facility.permit_number || '', c2+P, y+24, half-P*2);
    lbl('Issued by', c2+P, y+27);
    sf('normal',6); rc(); doc.text('Environment Agency', c2+P+14, y+27);
    y += c1H;
    ...
    // Waste-hierarchy declaration — full width
    const declH = 8;
    bg(margin, y, cW, declH, 240,250,242);
    dc(140,185,145,0.3); doc.rect(margin, y, cW, declH);
    sf('italic',6); sc(30,80,40);
    doc.text(
      ...
      margin+P, y+3.5, { maxWidth: cW-18 }
    );
    tick(margin+cW-10, y+2.5, true);
    sf('normal',6); rc(); doc.text('Yes', margin+cW-6, y+5);
    y += declH;

    // D1 — collection address | date | time
    const d1H = 11;
    box(margin, y, half, d1H);
    lbl('D1 Address of transfer or collection point', margin+P, y+3.5);
    val(form.collectionAddress || form.transferorAddress || '', margin+P, y+8, half-P*2);

    const dW = half*0.55 - 0.5;
    const tW = half - dW - 1;
    box(c2, y, dW, d1H);
    lbl('Date of transfer (DD/MM/YYYY)', c2+P, y+3.5, dW-P*2);
    val(transferDate, c2+P, y+8, dW-P*2);

    box(c2+dW+1, y, tW, d1H);
    lbl('Time(s)', c2+dW+1+P, y+3.5);
    val(form.transferTime, c2+dW+1+P, y+8, tW-P*2);
    y += d1H;

    // D2 — broker details (full width now process is in Section A)
    const d2H = 12;
    box(margin, y, cW, d2H);
    lbl('D2 Broker or dealer who arranged this transfer (if applicable)', margin+P, y+3.5, cW-P*2);
    val(form.brokerName, margin+P, y+8, cW*0.45);
    if (form.brokerRegNumber) {
      lbl('Registration number', margin+cW*0.5, y+3.5);
      val(form.brokerRegNumber, margin+cW*0.5, y+8, cW*0.45);
    }
    y += d2H;

    // Signature blocks — Transferor | Transferee
    const sigH = 18;
    box(margin, y, half, sigH);
    lbl('Transferor\'s signature', margin+P, y+3.5);
    hln(margin+P+25, y+4, margin+half-P);
    lbl('Name', margin+P, y+9);
    hln(margin+P+8, y+9, margin+half-P);
    lbl('Representing', margin+P, y+14.5);
    hln(margin+P+18, y+14.5, margin+half-P);

    box(c2, y, half, sigH);
    lbl('Transferee\'s signature', c2+P, y+3.5);
    hln(c2+P+25, y+4, c2+half-P);
    lbl('Name', c2+P, y+9);
    hln(c2+P+8, y+9, c2+half-P);
    lbl('Representing', c2+P, y+14.5);
    hln(c2+P+18, y+14.5, c2+half-P);
    y += sigH;

    // ── Footer ────────────────────────────────────────────────────────────
    bg(margin, y, cW, 5, 235,248,237);
    sf('normal',5); sc(80,80,80);
    doc.text('WMC2A Version 3, August 2011  ·  Retain for minimum 2 years  ·  Environmental Protection Act 1990, s.34  ·  Duty of Care Regulations 1991', margin+P, y+3.3);
    sf('bold',5); sc(30,90,50);
    doc.text('WasteLocate.co.uk', W-margin-P, y+3.3, { align:'right' });
    rc();
  };
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {field('Process giving rise to this waste (A4)', 'wasteProcess', 'e.g. building demolition, manufacturing process')}
                <div className="flex gap-2">
